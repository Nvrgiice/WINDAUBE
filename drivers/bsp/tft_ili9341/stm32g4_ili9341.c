...
void ILI9341_PutsCenter(uint16_t x, uint16_t y, char *str, FontDef_t *font, uint16_t foreground, uint16_t background) {
	uint16_t startX = x;

	/* Set X and Y coordinates as the middle of the textzone */
	uint16_t text_width = strlen(str) * font->FontWidth;
	uint16_t text_height = font->FontHeight;

	ILI9341_x = x - text_width/2;
	ILI9341_y = y - text_height/2;
